...
MQTT is a widely used lightweight messaging protocol for IoT, industrial telemetry, and event-driven systems. This design adds a new Drasi Source that subscribes to one or more MQTT topics and converts incoming messages into source changes that Drasi continuous queries can process in near real time. With this source, teams can connect devices and event publishers that already use MQTT brokers (for example, Mosquitto, EMQX, or HiveMQ)without building a custom bridge.

The proposed MQTT Source will handle broker connection configuration, secure authentication, topic subscription patterns, payload formats decoding, and reliable delivery semantics needed for continuous query processing. The result is a reusable, production ready ingestion path that expands Drasi’s connectivity for streaming scenarios.
